# frontend
Padrões e boas práticas para o Front-End

## CSS

### Sass
- **Pre-processor:** sintaxe scss
 - [Documentação](http://sass-lang.com/)
- **Guia de boas práticas:**
  - [Mdo Code Guide](http://diegoeis.github.io/code-guide/)

### Arquitetura
- **BEM:**
  - [Tutorial](https://css-tricks.com/bem-101/)
  - [BEM, um novo método para seu CSS](http://tableless.com.br/bem-um-novo-metodo-para-seu-css/)
- **SMACSS:**
  - [Web Book](https://smacss.com/)
- **Referencias:**
  - [OOCSS, SMACSS, BEM, DRY CSS: afinal, como escrever CSS?](http://tableless.com.br/oocss-smacss-bem-dry-css-afinal-como-escrever-css/)

### Linter
- **Sass-lint:**
  - [Documentação](https://github.com/brigade/scss-lint)
- **Plugins:**
  - [Atom](https://atom.io/packages/linter-scss-lint)
  - [Sublime](https://github.com/attenzione/SublimeLinter-scss-lint)


## Javascript

### ES6
- *Utilizar ES6 nos projetos novos*
- **ganho padrão para o js**: data-js-funcionalidade
- **Exemplos e tutoriais:**
  - [es6-cheatsheet](https://github.com/DrkSephy/es6-cheatsheet)
  - [es6features](https://github.com/lukehoban/es6features)
  - [ES6-Learning](https://github.com/ericdouglas/ES6-Learning)
- **Guia de boas práticas:**
  - [Airbnb ES6](https://github.com/airbnb/javascript)
  - [Airbnb ES5 pt-br](https://github.com/armoucar/javascript-style-guide)

### jQuery
- **Guia boas práticas:**
  - [jquery-standards](http://lab.abhinayrathore.com/jquery-standards/)

### Linter
- **ESlint:**
  - [Documentação](https://github.com/eslint/eslint)
- **Plugins:**
  - [Atom](https://github.com/AtomLinter/linter-eslint)
  - [Sublime](https://github.com/roadhump/SublimeLinter-eslint)
  - [Eclipse](http://eslint.org/docs/user-guide/integrations)


## Boas referências para pesquisas
- [MDN - Mozilla Developer Network](https://developer.mozilla.org/pt-BR/)
- [CSS Tricks](http://css-tricks.com/)
